<template>
    <div class="water-management-container">
        ...
        <template v-if="viewTab === 'Dashboard'">
            ...
            <section class="grid kpi-grid">
                <div class="card kpi">
                    ...
                    <div class="kpi-sub muted">From series x current source mix</div>
                </div>
                ...
                <div class="card kpi">
                    <div class="kpi-top">
                        ...
                        <div class="kpi-label">Current Campus Water Usage</div>
                    </div>
                ...
                <div class="card kpi">
                    <div class="kpi-top">
                        ...
                        <button class="kpi-edit-btn" @click="openBaiEditor">
                    ...
                    <div class="modal modal-bai">
                        ...
                        <div class="modal-body">
                            <label class="modal-label">
                                BAI value
                                <input type="number" v-model.number="baiDraftValue" class="modal-input" min="1" />
                            </label>
                            <p class="modal-hint">
                                Default BAI loads from public/water_bai.json. Edits are saved locally in this browser.
                            </p>
                        </div>
                        ...
                        <div class="modal-actions">
                            ...
                            <button class="modal-btn" @click="saveBaiValue">
                ...
                <div class="card chart-card">
                    <div class="card-header">
                        ...
                        <div class="chart-gran">
                            <button class="chip" :class="{ active: wmGran === 'H' }" @click="setGran('H')">H</button>
                            ...
                            <template v-if="wmGran === 'H'">
                                <input class="chip" type="date" v-model="wmDay" :max="todayLocalISODate()" />
                                <input class="chip" type="time" v-model="wmFromTime" />
                                <input class="chip" type="time" v-model="wmToTime" />
                            </template>
                            <template v-else-if="wmGran === 'D'">
                                <span class="muted">From</span>
                                <input class="chip" type="date" v-model="wmDayFrom" :max="todayLocalISODate()" />
                                <span class="muted">To</span>
                                <input class="chip" type="date" v-model="wmDayTo" :max="todayLocalISODate()" />
                            </template>
                            <template v-else-if="wmGran === 'W'">
                                <span class="muted">From</span>
                                <input class="chip" type="week" v-model="wmWeekFrom" />
                                <span class="muted">To</span>
                                <input class="chip" type="week" v-model="wmWeekTo" />
                            </template>
                    ...
                    <div class="chart-wrap" v-else-if="trendMode === 'line'">
            ...
            <section class="grid">
                <div class="card">
                    <div class="card-header">
                        ...
                        <small v-else-if="metersError" class="err">Error: {{ metersError }}</small>
                        <small v-if="thresholdsLoading" class="muted"> - Loading thresholds...</small>
                        <small v-else-if="thresholdsError" class="err"> - {{ thresholdsError }}</small>
                    </div>
                    <div class="table-wrap">
                        <table>
                            <thead>
        ...
        <!-- OK Success (Saved) modal   auto hides after 1.5s -->
...
const wmGran = ref('H')
const wmDay = ref(todayLocalISODate())
const wmDayFrom = ref(todayLocalISODate())
const wmDayTo = ref(todayLocalISODate())
const wmFromTime = ref('00:00')
const wmToTime = ref('23:00')
const wmWeek = ref(getCurrentISOWeek())
const wmWeekFrom = ref(getCurrentISOWeek())
const wmWeekTo = ref(getCurrentISOWeek())
...
function isoWeekKeyFromDate(dateObj) {
    return getCurrentISOWeekForDate(new Date(dateObj))
}

function getCurrentISOWeekForDate(dateObj) {
    const d = new Date(dateObj)
    const dow = d.getDay() || 7
    const thurs = new Date(d)
    thurs.setDate(d.getDate() + 4 - dow)
    const year = thurs.getFullYear()
    const yearStart = new Date(year, 0, 1)
    const week = Math.ceil((((thurs - yearStart) / 86400000) + 1) / 7)
    return `${year}-W${String(week).padStart(2, '0')}`
...
function listWeeksInclusive(startWeek, endWeek) {
    ...
    while (cur <= endStart) {
        out.push(isoWeekKeyFromDate(cur))
...
// X-axis tick formatter per H/D/W/M/Y (safe)
// X-axis tick formatter per H/D/W/M/Y (robust)
...
function fmtWaterTick(lbl) {
    ...
    if (!Number.isFinite(d.getTime())) return String(lbl) // OK safe guard

    if (wmGran.value === 'H') return _fmtTime.format(d)
...
const chartOptions = computed(() => ({
    ...
    layout: {
        padding: {
            bottom: 24   //  THIS fixes the x-axis overlap
        }
    },
    ...
    scales: {
        x: {
            ticks: {
                color: '#cbd5e1',
                padding: 8,      //  extra breathing room
                callback: (_v, idx) => {
                    const labels = chartA.value?.labels || []
                    return labels[idx] ? fmtWaterTick(labels[idx]) : ''
                }
            }
        },
        y: {
...
async function loadSpecialNeWaterSeries() {
    ...
    try {
        ...
        if (gran === 'H') {
            interestStartYmd = wmDay.value || today
            interestEndYmdEx = addDaysYmd(interestStartYmd, 1)
        } else if (gran === 'D') {
            interestStartYmd = wmDayFrom.value || today
            interestEndYmdEx = addDaysYmd(wmDayTo.value || interestStartYmd, 1)
        } else if (gran === 'W') {
            const { start } = weekBounds(wmWeekFrom.value) // Date objects, end exclusive
            const { end } = weekBounds(wmWeekTo.value)
        ...
        // Remapped chart views: D is daily From/To, W is weekly From/To.
        if (gran === 'H') {
            const day = wmDay.value || today
            const startHour = Number(String(wmFromTime.value || '00:00').slice(0, 2))
            const endHour = Number(String(wmToTime.value || '23:00').slice(0, 2))
            for (let h = startHour; h <= endHour; h += 1) {
                const hh = String(h).padStart(2, '0')
                labels.push(`${day}T${hh}:00:00`)
                pubData.push(0)
                neData.push(hourlyMap.get(`${day} ${hh}`) || 0)
            }
        } else if (gran === 'D') {
            labels.length = 0
            pubData.length = 0
            ...
            let cur = new Date(`${interestStartYmd}T00:00:00`)
            const end = new Date(`${interestEndYmdEx}T00:00:00`)
            while (cur < end) {
                const dayKey = ymd(cur)
                if (dayKey > today) break
                labels.push(dayKey)
                pubData.push(0)
                neData.push(dailyMap.get(dayKey) || 0)
                cur.setDate(cur.getDate() + 1)
            }
        } else if (gran === 'W') {
            labels.length = 0
            ...
            dailyMap.forEach((usage, day) => {
                const wk = isoWeekKeyFromDate(`${day}T00:00:00`)
                weekly.set(wk, (weekly.get(wk) || 0) + usage)
            })

            for (const week of listWeeksInclusive(wmWeekFrom.value, wmWeekTo.value)) {
                labels.push(labelWeek(week))
                pubData.push(0)
                neData.push(weekly.get(week) || 0)
            }
        }
        ...
        else if (gran === 'W') unitLabel = 'm³ / week'
        else unitLabel = 'm³ / day'
...
async function addSpecialNeToDailyChart(startYmd, endYmdEx) {
    if (!(currentTab.value === 'Overall' && currentLevel.value === 'Overall')) return
    ...
    const neIdx = 1
    const baseNe = Array.isArray(datasets[neIdx].data) ? datasets[neIdx].data.slice() : []
    const newNe = labels.map((lab, idx) => {
        const key = String(lab).slice(0, 10)
        return Number(baseNe[idx] || 0) + (extraDaily.get(key) || 0)
    })
...
function aggregateDailyChartToWeeks(startWeek, endWeek) {
    const chart = chartA.value || {}
    const labels = Array.isArray(chart.labels) ? chart.labels : []
    const datasets = Array.isArray(chart.datasets) ? chart.datasets : []
    const weeks = listWeeksInclusive(startWeek, endWeek)

    const aggregated = datasets.map(ds => {
        const bucket = new Map()
        const values = Array.isArray(ds.data) ? ds.data : []

        labels.forEach((label, idx) => {
            const dateKey = String(label).slice(0, 10)
            if (!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) return
            const weekKey = isoWeekKeyFromDate(`${dateKey}T00:00:00`)
            bucket.set(weekKey, (bucket.get(weekKey) || 0) + Number(values[idx] || 0))
        })

        return {
            ...ds,
            label: String(ds.label || '').replace(/daily\s*/i, 'weekly '),
            data: weeks.map(week => bucket.get(week) || 0),
        }
    })
...
async function loadWaterDWMy() {
    ...
    try {
        if (wmGran.value === 'H') {
            await loadHourlySeriesForActiveTab()
            return
        }

        /* ---------- DAILY RANGE ---------- */
        if (wmGran.value === 'D') {
            const start = new Date(`${wmDayFrom.value}T00:00:00`)
            const end = new Date(`${wmDayTo.value}T00:00:00`)
            end.setDate(end.getDate() + 1)
            const fetchStart = new Date(start)
            fetchStart.setDate(fetchStart.getDate() - 1)
            startLocal.value = toLocalYmdHm(fetchStart)
            endLocal.value = toLocalYmdHm(end)
            displayStartLocal.value = toLocalYmdHm(start)
            displayEndLocal.value = toLocalYmdHm(end)

            // build PUB + NEWater daily from normal meters
            await loadSeriesForActiveTab()

            await addSpecialNeToDailyChart(ymd(start), ymd(end))
            return
        }

        /* ---------- WEEKLY RANGE ---------- */
        if (wmGran.value === 'W') {
            const from = weekBounds(wmWeekFrom.value)
            const to = weekBounds(wmWeekTo.value)
            const fetchStart = new Date(from.start)
            fetchStart.setDate(fetchStart.getDate() - 1)
            startLocal.value = toLocalYmdHm(fetchStart)
            endLocal.value = toLocalYmdHm(to.end)
            displayStartLocal.value = toLocalYmdHm(from.start)
            displayEndLocal.value = toLocalYmdHm(to.end)

            await loadSeriesForActiveTab()
            await addSpecialNeToDailyChart(ymd(from.start), ymd(to.end))
            aggregateDailyChartToWeeks(wmWeekFrom.value, wmWeekTo.value)
...
/* Auto-apply when the period controls change. */
let waterPeriodTimer = null
function scheduleWaterPeriodApply() {
    if (waterPeriodTimer) clearTimeout(waterPeriodTimer)
    waterPeriodTimer = setTimeout(() => {
        waterPeriodTimer = null
        loadWaterDWMy()
    }, 120)
}

onMounted(() => { loadWaterDWMy() })
onUnmounted(() => { if (waterPeriodTimer) clearTimeout(waterPeriodTimer) })
watch([
    wmGran,
    wmDay,
    wmDayFrom,
...
async function buildTrendForTags(tags, startISO, endISO) {
    if (!tags?.length) return { labels: [], pub: [], ne: [], cutIndex: 0 }

    // OK Fetch 1 day BEFORE start so first in-range day can diff against previous day
    const startD = new Date(startISO)
    startD.setDate(startD.getDate() - 1)
    const fetchStartISO = startD.toISOString()

    // fetch per-tag series (with expanded window)
    const seriesMap = {}
    for (const t of tags) {
        seriesMap[t] = await fetchSeries(t, fetchStartISO, endISO) // uses your existing fetchSeries
    }

    // union timeline
    const timeline = unionSortedTimestamps(seriesMap) // your existing helper

    // find where "real range" starts (first timestamp >= original startISO)
    const startMs = new Date(startISO).getTime()
    let cutIndex = 0
    while (cutIndex < timeline.length) {
        const ms = new Date(timeline[cutIndex]).getTime()
        if (Number.isFinite(ms) && ms >= startMs) break
        cutIndex++
    }

    // aggregate PUB vs NEWater cumulative arrays
    const pubArr = [], neArr = []
    for (const ts of timeline) {
        let pub = 0, ne = 0
        for (const tag of tags) {
            const arr = seriesMap[tag] || []
            const val = safeGetValueAtTs(arr, ts) // your existing helper
    ...
    // labels aligned to timeline
    const labels = timeline.map(ts => new Date(ts).toLocaleString())

    // OK return cutIndex so export can skip the extra "previous day" row but still use it for diff
    return { labels, pub: pubArr, ne: neArr, cutIndex }
}


/** create a Trend section AOA:
 *  Timestamp | PUB (m³) | NEWater (m³) | Total (m³) | PUB Daily (m³) | NEWater Daily (m³) | Total Daily (m³)
 *
 * Assumes trend.pub / trend.ne are cumulative series aligned to trend.labels
 */
function trendToAOA(title, trend) {
    ...
    const pub = Array.isArray(trend?.pub) ? trend.pub : []
    const ne = Array.isArray(trend?.ne) ? trend.ne : []
    const cut = Number.isFinite(trend?.cutIndex) ? trend.cutIndex : 0
    ...
    // start from the "real" range start
    for (let i = cut; i < labels.length; i++) {
        const p = safeNum(pub[i])
        const n = safeNum(ne[i])
        const total = p + n

        // daily = current - previous (previous exists because we fetched 1 day earlier)
        const pPrev = (i > 0) ? safeNum(pub[i - 1]) : null
        const nPrev = (i > 0) ? safeNum(ne[i - 1]) : null

        const pDaily = (pPrev == null) ? 0 : Math.max(0, p - pPrev)
        const nDaily = (nPrev == null) ? 0 : Math.max(0, n - nPrev)
        const totalDaily = pDaily + nDaily

        aoa.push([labels[i], p, n, total, pDaily, nDaily, totalDaily])
    }
...
const summaryDateLocal = ref(todayLocalISODate())
...
const endLocal = ref('')
const displayStartLocal = ref('')
const displayEndLocal = ref('')
...
const baiValue = ref(DEFAULT_BAI_VALUE)
...
function syncBaiValueFromStorage(event) {
    ...
    try {
        ...
        console.warn('[Water] Failed to sync BAI value', e)
    }
}
...
function applyBaiValue(value) {
    baiValue.value = normalizeBaiValue(value)
    if (!isBaiEditOpen.value) baiDraftValue.value = baiValue.value
}
...
function openBaiEditor() {
    baiDraftValue.value = baiValue.value
    isBaiEditOpen.value = true
}

function saveBaiValue() {
    const next = normalizeBaiValue(baiDraftValue.value)
    baiValue.value = next
    baiDraftValue.value = next
    try {
        persistBaiValue(next)
    } catch (e) {
        console.warn('[Water] Failed to save local BAI value', e)
    }
...
async function loadBaiValue() {
    try {
        ...
        if (stored) {
            ...
            baiValue.value = normalizeBaiValue(parsed?.bai ?? parsed)
            baiDraftValue.value = baiValue.value
            return
        }
    } catch (e) {
        console.warn('[Water] Failed to read local BAI value', e)
    }
    ...
    try {
        ...
        baiValue.value = normalizeBaiValue(config?.bai)
        baiDraftValue.value = baiValue.value
    } catch (e) {
        console.warn('[Water] Failed to load BAI config, using default', e)
        baiValue.value = DEFAULT_BAI_VALUE
        baiDraftValue.value = baiValue.value
    }
}
...
onUnmounted(() => window.removeEventListener('storage', syncBaiValueFromStorage))
onUnmounted(() => window.removeEventListener(BAI_CHANGE_EVENT, syncBaiValueFromApp))
...
async function loadHourlySeriesForActiveTab() {
    seriesLoading.value = true
    seriesError.value = null
    try {
        const tags = activeMeterTags.value
        ...
        const start = new Date(`${wmDay.value}T${wmFromTime.value || '00:00'}:00`)
        const end = new Date(`${wmDay.value}T${wmToTime.value || '23:00'}:00`)
        if (!Number.isFinite(start.getTime()) || !Number.isFinite(end.getTime()) || start > end) {
            throw new Error('Invalid hourly time range')
        }

        const fetchStart = new Date(start)
        fetchStart.setHours(fetchStart.getHours() - 1)
        const fetchEnd = new Date(end)
        fetchEnd.setHours(fetchEnd.getHours() + 1)

        const seriesMap = {}
        for (const tag of tags) {
            seriesMap[tag] = await fetchSeries(tag, toLocalYmdHms(fetchStart), toLocalYmdHms(fetchEnd))
        }

        const timestamps = unionSortedTimestamps(seriesMap)
        const pubCum = []
        const neCum = []
        ...
        for (const ts of timestamps) {
            ...
            let ne = 0
            for (const tag of tags) {
                const v = safeGetValueAtTs(seriesMap[tag], ts)
                if ((meterTagToSource.value[tag] || 'PUB') === 'NEWater') ne += v
                else pub += v
            }
            pubCum.push(pub)
            neCum.push(ne)
        ...
        for (let i = 1; i < timestamps.length; i += 1) {
            ...
            pubData.push(Math.max(0, Number(pubCum[i] || 0) - Number(pubCum[i - 1] || 0)))
            neData.push(Math.max(0, Number(neCum[i] || 0) - Number(neCum[i - 1] || 0)))
        }

        chartA.value = {
            labels,
            datasets: [
                { label: 'PUB (m³ / hour)', data: pubData },
                { label: 'NEWater (m³ / hour)', data: neData }
            ]
        }
    } catch (e) {
        seriesError.value = e?.message ?? 'Hourly series load failed'
        chartA.value = { labels: [], datasets: [] }
    } finally {
        seriesLoading.value = false
    }
...
async function loadSeriesForActiveTab() {
    ...
    try {
        ...
        const startISO = startLocal.value ? toLocalYmdHms(new Date(startLocal.value)) : (defaultStartISO() + 'Z')
        const endISO = endLocal.value ? toLocalYmdHms(new Date(endLocal.value)) : (defaultEndISO() + 'Z')
        ...
        let visibleLabels = days
        let visiblePubDaily = pubDaily
        let visibleNeDaily = neDaily

        if (displayStartLocal.value && displayEndLocal.value) {
            const displayStart = new Date(displayStartLocal.value)
            const displayEnd = new Date(displayEndLocal.value)
            const keepIndexes = days
                .map((day, index) => ({ day, index, date: new Date(`${day}T00:00:00`) }))
                .filter(item => item.date >= displayStart && item.date < displayEnd)
                .map(item => item.index)

            visibleLabels = keepIndexes.map(index => days[index])
            visiblePubDaily = keepIndexes.map(index => pubDaily[index])
            visibleNeDaily = keepIndexes.map(index => neDaily[index])
        }

        chartA.value = {
            labels: visibleLabels, // keep as 'YYYY-MM-DD'
            datasets: [
                { label: 'PUB (daily m³)', data: visiblePubDaily },
                { label: 'NEWater (daily m³)', data: visibleNeDaily }
            ]
        }
...
function buildDeviceListCsv() {
    ...
    const HEADERS = [
        'Block', 'Level', 'Meter Tag', 'Place', 'Latest Cumulative', 'Section', 'Source', 'Remarks'
    ]
...
function buildTrendCsv() {
    ...
    const labels = chartA.value?.labels || []
    const ds = chartA.value?.datasets || []
    const pubCum = ds[0]?.data || []
    const neCum = ds[1]?.data || []

    const HEADERS = ['Timestamp', 'PUB (m3)', 'NEWater (m3)', 'Total (m3)']

    const diffClamp = (cur, prev) => {
        const a = Number(cur)
        const b = Number(prev)
        if (!Number.isFinite(a)) return 0
        if (!Number.isFinite(b)) return 0
        const d = a - b
        // if meter resets / rollovers happen, avoid negative daily usage
        return d >= 0 ? d : 0
    }

    const rows = labels.map((ts, i) => {
        // daily = today cumulative - yesterday cumulative
        const pDaily = i === 0 ? 0 : diffClamp(pubCum[i], pubCum[i - 1])
        const nDaily = i === 0 ? 0 : diffClamp(neCum[i], neCum[i - 1])

        return {
            'Timestamp': ts,
            'PUB (m3)': pDaily,
            'NEWater (m3)': nDaily,
            'Total (m3)': pDaily + nDaily
        }
    })
...
function toAOA(rows) {
    const header = ['Block', 'Level', 'Meter Tag', 'Place', 'Latest Cumulative', 'Section', 'Source', 'Remarks']
    const aoa = [header]
    const dashIfEmpty = (v) => {
        if (v === 0) return 0

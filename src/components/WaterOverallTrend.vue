<template>
    <section class="grid charts-grid">
        ...
        <div class="top-kpi-row">
            ...
            <div class="kpi-card kpi-card-inline">
                <div class="kpi-header">
                    ...
                    <button class="chip chip-ghost" @click="openBaiEdit">
                        Edit
                    </button>
                </div>
                <div class="kpi-value">
        ...
        <!-- === Clean header with only "Open Chart" === -->
        <div class="card chart-card">
            <div class="card-header card-head-actions">
                <h3>Water — Consumption Trend</h3>
                <div class="spacer"></div>
                <button class="chip" @click="openFs">Open Chart</button>
            </div>

            <!-- Optional loading/error message (no inline chart) -->
            <div class="chart-wrap" v-if="seriesLoading">Loading…</div>
            <div class="chart-wrap" v-else-if="seriesError"><span class="err">{{ seriesError }}</span></div>
            <div class="chart-wrap" v-else style="display:none;"></div>
        </div>
            ...
            <div v-if="isBaiEditOpen" class="bai-modal-backdrop" @click.self="isBaiEditOpen = false">
                <div class="bai-modal">
                    <div class="bai-modal-header">
                        <h3>Edit BAI</h3>
                        <span class="bai-modal-subtitle">
                            Business Activity Indicator used for WEI.
                        </span>
                    </div>
                    ...
                    <div class="bai-modal-actions">
                        <button class="bai-modal-btn ghost" @click="isBaiEditOpen = false">
                            ✕ Close
                        </button>
                        <button class="bai-modal-btn primary" @click="saveBai">
                            Save
                        </button>
                    </div>
                </div>
        ...
        <!-- ===== Fullscreen Modal (only place we render charts) ===== -->
        <Teleport to="body">
                    ...
                    <div class="fs-modal-body">
                        ...
                        <div class="trend-controls-row">
                            <div class="trend-actions">
                                <button class="chip" :class="{ active: wmGran === 'H' }"
                                    @click="setGran('H')">Hourly</button>
                                <button class="chip" :class="{ active: wmGran === 'D' }"
                                    @click="setGran('D')">Daily</button>
                                <button class="chip" :class="{ active: wmGran === 'W' }"
                                    @click="setGran('W')">Weekly</button>
                                <button class="chip" :class="{ active: wmGran === 'M' }"
                                    @click="setGran('M')">Monthly</button>
                                <button class="chip" :class="{ active: wmGran === 'Y' }"
                                    @click="setGran('Y')">Yearly</button>

                                <button class="chip" v-if="wmGran === 'D' && !seriesLoading" @click="toggleTrendMode"
...
const emit = defineEmits(['update:baiValue', 'chart-updated', 'ui-updated'])
...
function listWeeksInclusive(startWeek, endWeek) {
    const out = []
    const startInfo = weekBounds(startWeek)
    const endInfo = weekBounds(endWeek)
    if (!(startInfo.start instanceof Date) || !(endInfo.start instanceof Date)) return out
    if (Number.isNaN(startInfo.start.getTime()) || Number.isNaN(endInfo.start.getTime())) return out
    let cursor = new Date(startInfo.start)
    let last = new Date(endInfo.start)
    if (cursor > last) [cursor, last] = [last, cursor]
    while (cursor <= last) {
        out.push(getISOWeekString(cursor))
...
function labelWeek(weekKey) {
    ...
    const weekNo = String(weekKey || '').split('-W')[1] || ''
    return `Week ${Number(weekNo) || weekNo} (${_fmtDate.format(start)})`
}
...
function toLocalYmdHms(d) { return `${toLocalYmdHm(d)}:00` }
...
const wmToTime = ref('23:59')
const wmWeek = ref(isMondayToday() ? getYesterdayISOWeek() : getCurrentISOWeek())
const wmDayFrom = ref(todayLocalISODate())
const wmDayTo = ref(todayLocalISODate())
const wmWeekFrom = ref(isMondayToday() ? getYesterdayISOWeek() : getCurrentISOWeek())
const wmWeekTo = ref(isMondayToday() ? getYesterdayISOWeek() : getCurrentISOWeek())
...
function normalizeGran(g) {
    return ({ hour: 'H', hourly: 'H', day: 'D', week: 'W', month: 'M', year: 'Y' }[g] || g || 'H')
}
function setGran(g) {
    wmGran.value = normalizeGran(g)
    emitUiSnapshot()
}
...
const headerDateLabel = computed(() => {
    if (wmGran.value === 'H') return `${_fmtDay.format(new Date(wmDay.value))} ${wmFromTime.value} - ${wmToTime.value}`
    if (wmGran.value === 'D') return `${_fmtDay.format(new Date(wmDayFrom.value))} – ${_fmtDay.format(new Date(wmDayTo.value))}`
    if (wmGran.value === 'W') {
        const from = labelWeek(wmWeekFrom.value)
        const to = labelWeek(wmWeekTo.value)
...
function updateTopKpisFromChart() {
    // The KPI cards are "today" cards and must match WaterManagement.vue.
    // Keep them independent from the currently selected chart period.
    return
}
...
async function loadTodayKpis() {
    ...
    const prevYmd = ymdPrev(today)
    const { startISO, endISO } = dayWindowISO(today)
    ...
    for (let i = 0; i < tags.length; i++) {
        ...
        const prev = Number(dailyCum.get(prevYmd) ?? 0)
        const current = Number(dailyCum.get(today) ?? prev)
        ...
        tooltip: {
            ...
            callbacks: {
                title: (items) => {
                    ...
                    if (wmGran.value === 'H' && Number.isFinite(d.getTime())) {
                        return `${_fmtDay.format(d)} ${_fmtTime.format(d)}`
                    }
...
async function loadMeters() {
    try { const data = await fetchJson(METERS_URL, { cache: 'no-cache' }); flatRows.value = flattenMeters(data) }
...
async function loadSeriesOverallWeekly(start, end, fetchStartOverride = null) {
    ...
    // Dates used for fetching. This can start before the visible range so the
    // first visible day has a previous cumulative reading for its delta.
    const startDateObj = fetchStartOverride ? new Date(fetchStartOverride) : (start ? new Date(start) : new Date(Date.now() - 7 * 86400e3))
    const endDateObj = end ? new Date(end) : new Date()

    const startISO = toLocalYmdHms(startDateObj)
    const endISO = toLocalYmdHms(endDateObj)
    ...
    if (displayStart && displayEnd) {
        ...
        while (cursor < displayEnd) {
            ...
            for (const t of tags) {
                const m = dailyCumByTag[t]
                if (!m) continue
                const current = cumAtOrBefore(m, dayKey)
                const previous = cumBefore(m, dayKey)
                const delta = Math.max(0, Number(current || 0) - Number(previous || 0))
                if (isNE(t)) ne += delta
                else pub += delta
            }
...
function aggregateDailyChartToWeeks(startWeek, endWeek) {
    const labels = Array.isArray(chartA.value?.labels) ? chartA.value.labels : []
    const ds = Array.isArray(chartA.value?.datasets) ? chartA.value.datasets : []
    if (!labels.length || !ds.length) return

    const weekKeys = listWeeksInclusive(startWeek, endWeek)
    const totals = new Map(weekKeys.map((weekKey) => [weekKey, 0]))

    labels.forEach((label, index) => {
        const weekKey = getISOWeekString(new Date(label))
        if (!totals.has(weekKey)) return
        const totalForDay = ds.reduce((sum, dataset) => sum + Number(dataset?.data?.[index] || 0), 0)
        totals.set(weekKey, Number(totals.get(weekKey) || 0) + totalForDay)
    })

    chartA.value = {
        labels: weekKeys.map(labelWeek),
        datasets: [
            {
                label: 'm3 / week',
                type: 'bar',
                data: weekKeys.map((weekKey) => Number(totals.get(weekKey) || 0)),
                ...DS_COLORS.total,
            },
        ],
    }
...
async function loadHourlyOverall(start, end) {
    const tags = allMeterTags.value
    if (!tags.length) {
        chartA.value = { labels: [], datasets: [] }
        seriesError.value = seriesError.value || 'No meters found (check /water_meters_combined_full_sorted.json)'
    ...
    const startISO = toLocalYmdHms(new Date(start))
    const endISO = toLocalYmdHms(new Date(end))
    const seriesArr = await Promise.all(tags.map(t => fetchSeries(t, startISO, endISO)))
    const seriesMap = Object.fromEntries(tags.map((t, i) => [t, seriesArr[i] || []]))
    const labels = Array.from(new Set(seriesArr.flatMap(rows => rows.map(row => row.ts).filter(Boolean))))
    ...
    for (const ts of labels) {
        ...
        for (const tag of tags) {
            const rows = seriesMap[tag]
            const idx = rows.findIndex(row => row.ts === ts)
            if (idx < 0) continue
            const current = Number(rows[idx]?.value || 0)
            const previous = idx > 0 ? Number(rows[idx - 1]?.value || 0) : current
            const usage = Math.max(0, current - previous)
            if (isNE(tag)) ne += usage
            else pub += usage
        }
        pubHourly.push(pub)
        neHourly.push(ne)
    ...
    chartA.value = {
        labels,
        datasets: [
            { label: 'PUB (m3 / hour)', type: 'line', data: pubHourly, tension: 0.25, fill: false, ...DS_COLORS.pub },
            { label: 'NEWater (m3 / hour)', type: 'line', data: neHourly, tension: 0.25, fill: false, ...DS_COLORS.ne },
        ],
    }
}
...
async function loadWaterDWMy() {
    ...
    try {
        if (wmGran.value === 'H') {
            ...
            const end = new Date(`${wmDay.value}T${wmToTime.value || '23:59'}:00`)
            const from = start <= end ? start : end
            const to = start <= end ? end : start
            startLocal.value = toLocalYmdHm(from)
            endLocal.value = toLocalYmdHm(to)
            await loadHourlyOverall(startLocal.value, endLocal.value)
            updateTopKpisFromChart()
            return
        }

        if (wmGran.value === 'D') {
            const fromDate = new Date(`${wmDayFrom.value}T00:00`)
            const toDate = new Date(`${wmDayTo.value}T00:00`)
            const start = fromDate <= toDate ? fromDate : toDate
            const end = fromDate <= toDate ? toDate : fromDate
            end.setDate(end.getDate() + 1)
            startLocal.value = toLocalYmdHm(start);
            endLocal.value = toLocalYmdHm(end);
            const fetchStartLocal = toLocalYmdHm(addDays(start, -1));

            await loadSeriesOverallWeekly(startLocal.value, endLocal.value, fetchStartLocal);
        ...
        if (wmGran.value === 'W') {
            const fromInfo = weekBounds(wmWeekFrom.value);
            const toInfo = weekBounds(wmWeekTo.value);
            const start = fromInfo.start <= toInfo.start ? fromInfo.start : toInfo.start;
            const end = fromInfo.start <= toInfo.start ? toInfo.end : fromInfo.end;
            startLocal.value = toLocalYmdHm(start);
            endLocal.value = toLocalYmdHm(end);
            const fetchStartLocal = toLocalYmdHm(addDays(start, -1));
...
const fsIsLine = computed(() => wmGran.value === 'H' || (wmGran.value === 'D' && trendMode.value === 'line'))
const fsChartData = computed(() => fsIsLine.value ? chartA.value : (wmGran.value === 'D' && trendMode.value !== 'line') ? compareBarDataSelection.value : chartA.value)
const fsChartOptions = computed(() => fsIsLine.value ? baseChartOptions.value : barOptions.value)

function getChartPayload() {
    return {
        chartData: fsChartData.value,
        chartOptions: fsChartOptions.value,
        loading: seriesLoading.value,
        isLine: fsIsLine.value,
    }
}
function getUi() {
    const modeMap = { H: 'hour', D: 'day', W: 'week', M: 'month', Y: 'year' }
    return {
        rangeMode: modeMap[wmGran.value] || 'day',
        dateLabel: headerDateLabel.value,
        canStepNext: false,
        selectedDay: wmDay.value,
        selectedFromTime: wmFromTime.value,
        selectedToTime: wmToTime.value,
        selectedDayFrom: wmDayFrom.value,
        selectedDayTo: wmDayTo.value,
        selectedWeek: wmWeek.value,
        selectedWeekFrom: wmWeekFrom.value,
        selectedWeekTo: wmWeekTo.value,
        selectedMonth: wmMonthStart.value,
        selectedMonthStart: wmMonthStart.value,
        selectedMonthEnd: wmMonthEnd.value,
        selectedYear: wmYearStart.value,
        selectedYearStart: wmYearStart.value,
        selectedYearEnd: wmYearEnd.value,
        granularityHint: wmGran.value === 'H' ? 'Hourly' : wmGran.value === 'D' ? 'Daily' : wmGran.value === 'W' ? 'Weekly' : wmGran.value === 'M' ? 'Monthly' : 'Yearly',
        isLine: fsIsLine.value,
    }
}
function emitChartSnapshot() { emit('chart-updated', getChartPayload()) }
function emitUiSnapshot() { emit('ui-updated', getUi()) }
function setRangeMode(mode) { setGran(mode) }
function applyFromUi() { return loadWaterDWMy() }
function setSelectedDay(value) { wmDayFrom.value = value; wmDayTo.value = value; wmDay.value = value; emitUiSnapshot() }
function setSelectedFromTime(value) { wmFromTime.value = value; emitUiSnapshot() }
function setSelectedToTime(value) { wmToTime.value = value; emitUiSnapshot() }
function setSelectedDayFrom(value) { wmDayFrom.value = value; wmDay.value = value; emitUiSnapshot() }
function setSelectedDayTo(value) { wmDayTo.value = value; emitUiSnapshot() }
function setSelectedWeek(value) { wmWeekFrom.value = value; wmWeekTo.value = value; wmWeek.value = value; emitUiSnapshot() }
function setSelectedWeekFrom(value) { wmWeekFrom.value = value; wmWeek.value = value; emitUiSnapshot() }
function setSelectedWeekTo(value) { wmWeekTo.value = value; emitUiSnapshot() }
function setSelectedMonth(value) { wmMonthStart.value = value; wmMonthEnd.value = value; emitUiSnapshot() }
function setSelectedYear(value) { wmYearStart.value = Number(value); wmYearEnd.value = Number(value); emitUiSnapshot() }
function stepPeriod() { }

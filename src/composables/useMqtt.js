...
function seedFakeHourlyData() {
  const now = new Date()
  now.setMinutes(0, 0, 0)

  for (const period of ['business', 'afterhours']) {
    for (let i = 47; i >= 0; i -= 1) {
      const dt = new Date(now)
      dt.setHours(now.getHours() - i)
      const hour = dt.getHours()
      const occupied = period === 'business' && hour >= 7 && hour <= 19
      const shape = occupied ? 1 : 0.42
      const trRef = occupied ? 1180 + Math.sin(i / 5) * 90 : 720 + Math.sin(i / 4) * 45
      const measured = trRef * (0.66 + Math.sin(i / 6) * 0.035) * shape
      const curve = trRef * 0.62 * shape
      pushHourly('airside', period, {
        ts_hour: dt.toISOString(),
        _ts_local: dt.toLocaleString([], {
          year: 'numeric',
          month: '2-digit',
          day: '2-digit',
          hour: '2-digit',
        ...
        kW_air_measured: Number(measured.toFixed(2)),
        kW_air_curve: Number(curve.toFixed(2)),
        air_kw_per_rt: Number((measured / Math.max(trRef, 1)).toFixed(3)),
        kWh_air_measured: Number(measured.toFixed(2)),
        kWh_air_curve: Number(curve.toFixed(2)),
        tr_ref: Number(trRef.toFixed(0)),
        data_valid: true,
      })
    }
  }
...
function ensureConnected() {
  ...
  client = {
    subscribe: () => {},
    unsubscribe: (_topic, cb) => { if (typeof cb === 'function') cb() },
    publish: (_topic, _message, _options, cb) => { if (typeof cb === 'function') cb(null) },
    on: () => {},
  }
